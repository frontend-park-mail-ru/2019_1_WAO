...
export function renderProfilePage(AjaxModule, me) {
  ...
  const profileContainerGridblock = document.createElement('div');
  ...
  const profileDeleteButtonPic = document.createElement('div');
  ...
  const profileDeleteButtonText = document.createElement('div');
    ...
    AjaxModule.doGet({
      callback(xhr) {
        if (!xhr.responseText) {
          ...
          Add_router.createNavbarProfile(AjaxModule);
          return;
        }

        const user = JSON.parse(xhr.responseText);
        application.innerHTML = '';
        Add_router.createNavbarProfile(user);
  ...
  listDivs.forEach((item) =>{
    ...
    const profileDataBottomBorderDiv = document.createElement('div');
